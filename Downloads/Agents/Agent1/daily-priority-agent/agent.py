...
# Step 7: Available time today (in minutes)
AVAILABLE_MIN = 120  # 2 hours per day
...
def build_plan(tasks: List[Task]) -> Dict:
    ...
    # Step 7: Apply available time constraint
    top3 = []
    next5 = []
    time_used = 0
    
    for i, (t, s, b) in enumerate(unblocked):
        if i < TOP3_COUNT:
            if time_used + t.effort_min <= AVAILABLE_MIN:
                top3.append((t, s, b))
                time_used += t.effort_min
            else:
                # If it doesn't fit in available time, move to next5
                next5.append((t, s, b))
        elif i < TOP3_COUNT + NEXT5_COUNT:
            next5.append((t, s, b))
        else:
            break
    ...
    plan = {
        ...
        "available_time_min": AVAILABLE_MIN,
        "time_allocated_min": time_used,
        "time_remaining_min": AVAILABLE_MIN - time_used,
        "top3": serialize(top3),
...
def render_summary(plan: Dict) -> str:
    ...
    lines.append("=" * 45)
    lines.append(f"Available Time: {plan['available_time_min']} min | Allocated: {plan['time_allocated_min']} min | Remaining: {plan['time_remaining_min']} min")
    ...
    def section(name: str, items: List[Dict]):
        ...
        for i, it in enumerate(items, 1):
            ...
            total_effort += it['effort_min']
        lines.append(f"   Total effort: {total_effort} min")
